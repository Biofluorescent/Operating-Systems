...
//	buffer[strcspn(buffer, "\n")] = '\0'; // Remove the trailing \n that fgets adds
	...
	if (charsWritten < strlen(text)) fprintf(stderr, "CLIENT: WARNING: Not all data written to socket!\n");
        ...
        // Get confimation from server that all text received before sending key. 
        memset(buffer, '\0', sizeof(buffer));
        charsRead = recv(socketFD, buffer, sizeof(buffer) - 1, 0);
